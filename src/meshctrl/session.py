...
class Session(object):
    ...
    async def add_users_to_device(self, userids, nodeid, rights=None, timeout=None):
        ...
        userids = [f"user//{u}" if not (u.startswith("user//") or u.startswith("ugrp//")) else u for u in userids]
        if rights is None:
            rights = 0
    ...
    async def add_users_to_device_group(self, userids, meshid, isname=False, rights=0, timeout=None):
        ...
        userids = [f"user//{u}" if not (u.startswith("user//") or u.startswith("ugrp//")) else u for u in userids]
        op = { "action": 'addmeshuser', "userids": userids, "meshrights": rights, "meshid": meshid };
        if isname:
            op["meshname"] = meshid
            del op["meshid"]
